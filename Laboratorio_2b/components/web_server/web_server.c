...
static esp_err_t led_on_handler_rojo(httpd_req_t *req)
{
    ESP_LOGI(TAG, "LED ON");
    rgb_led_rojo();
    httpd_resp_send(req, "ROJO", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
static esp_err_t led_on_handler_verde(httpd_req_t *req)
{
    ESP_LOGI(TAG, "LED ON");
    rgb_led_verde();
    httpd_resp_send(req, "VERDE", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
static esp_err_t led_on_handler_azul(httpd_req_t *req)
{
    ESP_LOGI(TAG, "LED ON");
    rgb_led_azul();
    httpd_resp_send(req, "AZUL", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
static esp_err_t led_on_handler_amarillo(httpd_req_t *req)
{
    ESP_LOGI(TAG, "LED ON");
    rgb_led_amarillo();
    httpd_resp_send(req, "AMARILLO", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
static esp_err_t led_on_handler_cyan(httpd_req_t *req)
{
    ESP_LOGI(TAG, "LED ON");
    rgb_led_cyan();
    httpd_resp_send(req, "CYAN", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
static esp_err_t led_on_handler_magenta(httpd_req_t *req)
{
    ESP_LOGI(TAG, "LED ON");
    rgb_led_magenta();
    httpd_resp_send(req, "MAGENTA", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
static esp_err_t led_on_handler_blanco(httpd_req_t *req)
{
    ESP_LOGI(TAG, "LED ON");
    rgb_led_blanco();
    httpd_resp_send(req, "BLANCO", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
static esp_err_t led_status_handler(httpd_req_t *req)
{
    ...
    snprintf(json, sizeof(json), "{\"on\":%s,\"color\":\"%s\"}",
